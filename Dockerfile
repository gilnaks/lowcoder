...
RUN --mount=type=cache,id=s/3f639f73-fed0-42c6-90dd-b6669ada0481-/root/.m2 mvn -f pom.xml,target=/root/.m2 mvn -f pom.xml clean package -DskipTests
...
RUN apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y curl ca-certificates build-essential gnupg
